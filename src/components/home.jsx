...
function Home() {
  ...
  const getUserData = async () => {
    ...
    FetchUserDataFromDb(setUserData, setLoading, setError);
  }
  ...
  const updateUserData = async (e, user_id, firstName, lastName, imgURL) => {
    e.preventDefault();
    const userImg = await getImageUrl(userImage);
    console.log(userImg);
  ...
  return (
    <>
      {
        error ?
          <h1><QuestionCircleTwoTone style={{ fontSize: 30 }} /> Sorry, we cannot fetch user data! Try again </h1> :
          isLoading === true ? <LoadingOutlined style={{ fontSize: 30, marginTop: "20px" }} /> :
